...
class yaraBot_chatController extends yaraBot_baseController {
    constructor() {
        ...
        this.mode = 'text'
        this.isMuted = false;
        this.session_id = localStorage.getItem("YARABOT_WIDGET_Conversation") || null;
        this.scroll = true;
        this.messageBuffer = "";
    ...
    async config() {
        ...
        if (localStorage.getItem("YARABOT_WIDGET_Conversation") != undefined) {
            ...
            if (currentTime - savedTime >= twentyFourHours) {
                ...
                let data = await this.GetChatData(localStorage.getItem("YARABOT_WIDGET_Conversation"));
                if (this.chatList == null) {
                    data?.chat_success_data.map((e) => {
                        if (e?.role === "user") {
                            const div = document.createElement('div');
                            div.classList.add('yarabot_Message');
                            div.classList.add('yarabot_UserMessage');
